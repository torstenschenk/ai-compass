...
class ClusterEngine:
    ...
    def predict(self, company_df):
        # Align features with Scaler using feature_names_in_ if available
        if hasattr(self.scaler, 'feature_names_in_'):
            expected_feats = self.scaler.feature_names_in_
            # Reindex adds missing cols with fill_value (1.0 for neutral/low) and ignores extras
            # Actually, fill_value=1.0 is safer for maturity (1-5 scale) than 0.
            company_df = company_df.reindex(columns=expected_feats, fill_value=1.0)
            
        X_new = self.scaler.transform(company_df.fillna(1.0))
...
class RoadmapGenerator:
    ...
    def generate(self, company_id, company_dim_series, company_question_df, strategic_gaps):
        ...
        # Align features
        if hasattr(self.scaler, 'feature_names_in_'):
             df = company_dim_series.to_frame().T
             df = df.reindex(columns=self.scaler.feature_names_in_, fill_value=1.0)
             X_in = self.scaler.transform(df)
        else:
